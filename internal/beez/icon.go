...
// tappableEmoji is a tap target that shows an emoji on the app background.
type tappableEmoji struct {
	widget.BaseWidget
	label *canvas.Text
	onTap func()
}

func newTappableEmoji(emoji string, onTap func()) *tappableEmoji {
	t := &tappableEmoji{onTap: onTap}
	t.label = canvas.NewText(emoji, colorAmberGlow)
	t.label.TextSize = 22
...
func (t *tappableEmoji) CreateRenderer() fyne.WidgetRenderer {
	bg := canvas.NewRectangle(colorInk)
	bg.SetMinSize(fyne.NewSize(36, 36))
	return widget.NewSimpleRenderer(container.NewStack(bg, container.NewCenter(t.label)))
}

func (t *tappableEmoji) Tapped(*fyne.PointEvent) {
	if t.onTap != nil {
		t.onTap()
	}
}

func (t *tappableEmoji) TappedSecondary(*fyne.PointEvent) {}
